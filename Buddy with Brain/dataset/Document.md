...
Task 1.1 (Kickoff): Conducted a project kickoff meeting to establish a comprehensive understanding of the project's goals, scope, and intended real-world applications.

Task 1.2 (Research): Studied fundamental AI/ML concepts, focusing specifically on time-series forecasting (ARIMA, Prophet) and financial data analysis.

Task 1.3 (User Stories): Analyzed and documented potential user stories for key demographics (students, families, small businesses) to align development with user-centric needs.

Week 2: Technical Upskilling and Environment Setup

Task 2.1 (Python Intensive): Completed a Python programming intensive to strengthen skills in essential data structures, algorithms, and logic.

Task 2.2 (Data Science Libraries): Gained hands-on experience with core data science libraries, mastering Pandas for data manipulation/cleaning and NumPy for numerical operations.

Task 2.3 (Visualization): Learned to generate insightful charts and graphs using Matplotlib and Seaborn, focusing on plotting time-series data and categorical distributions.

Task 2.4 (Environment): Set up the local development environment, including installing Python, VS Code, Git, and all necessary libraries (pandas, numpy, matplotlib, seaborn, streamlit, nltk).

Week 3: Data Sourcing and Initial Development

Task 3.1 (Tech Stack): Finalized the project's technology stack, confirming Python as the core language and Streamlit for the user interface and rapid prototyping.

Task 3.2 (Data Sourcing): Identified and acquired initial datasets (e.g., sample CSVs) for development and testing.

Task 3.3 (Data Cleaning): Commenced the first phase of development, writing initial scripts for data cleaning and preprocessing (e.g., handling missing values, standardizing date formats).

Task 3.4 (Initial Framework): Set up the basic Streamlit application structure and established the initial framework for model training and data ingestion.
...
Task 4.1 (Categorization Engine): Design and implement a rule-based system for initial transaction categorization using keyword matching (e.g., "Starbucks" -> "Food/Drink").

Task 4.2 (NLP Integration): Integrate the NLTK library for basic text processing (tokenization, stop-word removal) to improve the accuracy of keyword matching from transaction descriptions.

Task 4.3 (Manual Override): Implement the backend logic to allow users to manually override or correct an automatically assigned category.

Task 4.4 (Pandas Reporting): Develop core functions using Pandas (groupby(), sum(), resample()) to aggregate spending data and calculate totals per category and per month.

Task 4.5 (Income vs. Expense): Create a specific function to generate a summary of total income vs. total expenses for a given period.

Week 5: Dashboard Integration and Visualization

Task 5.1 (Dashboard View): Enhance the Streamlit UI to add an "Initial Dashboard View" or "Spending Summary" page.

Task 5.2 (Recent Transactions): Integrate a component to display a summary of recent transactions (e.g., last 10) with their newly assigned categories.

Task 5.3 (Visualization): Use Matplotlib/Seaborn to create the first set of visualizations:
...
Task 5.4 (UI Refinement): Connect the manual override functionality to the UI, allowing users to correct categories directly from the recent transactions list.
